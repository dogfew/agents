...
app.layout = html.Div([
    ...
    html.P("samples"),
    dcc.Input(id='num', type='number', min=2, max=1024, step=2, value=64, debounce=True),
    html.P("Bias + Amplitudes (sep='  ')"),
    dcc.Input(type='text', value='1 1 1 1', id='amplitudes', debounce=True),
    html.P("Phases (sep='  ')"),
    dcc.Input(type='text', value='1.57 0 0.42', id='phases', debounce=True),
    dcc.Graph(id="graph"),
...
def plot_sin(i, n_samples, amps, phases):
    ...
    signals = [a * np.sin(x * (f + 1) + phi) for f, (a, phi) in enumerate(zip(amps[1:], phases))]
